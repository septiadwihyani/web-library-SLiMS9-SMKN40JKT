...
function sendMessage($wa_no, $wa_text) {
    ...
    $data = array(
        'device_id' => '86512b053bd793e58d9af42724c35b46',
        'number' => $nohp,
        'message' => $pesan,
    ...
    <div id="receiptInfo">
        ...
        if (isset($_SESSION['receipt_record']['loan'])) {
            foreach ($_SESSION['receipt_record']['loan'] as $loan) {
                ...
                echo '</div>';
                $msgloan .= 'Kode : '.$loan['itemCode'].'
Judul : '.$loan['title'].'
Tanggal Pinjam (YYYY-MM-DD) : '.$loan['loanDate'].'
Tanggal Kembali (YYYY-MM-DD): '.$loan['dueDate'].'
...
Nama : '.$_SESSION['receipt_record']['memberName'].'
ID Anggota : '.$_SESSION['receipt_record']['memberID'].'
...
'.$msgloan.'';
        ...
        if (isset($_SESSION['receipt_record']['extend'])) {
            foreach ($_SESSION['receipt_record']['extend'] as $ext) {
                ...
                echo '</div>';
            $msgext .= 'Kode : '.$ext['itemCode'].'
Judul : '.$ext['title'].'
Tanggal Pinjam (YYYY-MM-DD) : '.$ext['loanDate'].'
Tanggal Kembali (YYYY-MM-DD): '.$ext['dueDate'].'
...
'.$msgext.'';
        ...
        foreach ($_SESSION['receipt_record']['return'] as $ret) {
            ...
            echo '</div>';
        $msgret .= 'Kode : '.$ret['itemCode'].'
Judul : '.$ret['title'].'
Tanggal dikembalikan (YYYY-MM-DD) : '.$ret['returnDate'].'
Keterlambatan : '.$ret['overdues']['days'].' hari
