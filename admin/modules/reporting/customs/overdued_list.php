...
echo simbio_form_element::dateField('untilDate', date('Y-m-d'),'class="form-control"');
  ...
  <?php
    ...
    function showOverduedList($obj_db, $array_data)
    {
        ...
        // member name
        $member_q = $obj_db->query('SELECT m.member_name, m.member_email, m.member_phone, mmt.fine_each_day 
                                           FROM member m 
        ...
        $_buffer .= '<div style="font-size: 10pt; margin-bottom: 3px;"><a class="usingAJAX" href="' . MWB . 'membership/overdue_wa.php' . '" postdata="memberID=' . $array_data[0] . '" loadcontainer="' . $array_data[0] . 'waStatus">' . __('Kirim Notifikasi WhatsApp') . '</a> - ' . __('Phone Number') . ': ' . $member_d[2] . '</div>';
