...
echo simbio_form_element::dateField('untilDate', date('Y-m-d'),'class="form-control"');
  ...
  <?php
    ...
    function showOverduedList($obj_db, $array_data)
    {
        ...
        $_buffer .= '<div style="font-size: 10pt; margin-bottom: 3px;"><a class="usingAJAX" href="' . MWB . 'membership/overdue_wa.php' . '" postdata="memberID=' . $array_data[0] . '" loadcontainer="' . $array_data[0] . 'emailStatus">' . __('Kirim Notifikasi WhatsApp') . '</a> - ' . __('Phone Number') . ': ' . $member_d[2] . '</div>';
